...
function ListPage({ lions, setLions, apiStatus, isPending, hasError, onFetch, onDeleteLast, onRetry }: ListPageProps) {
  ...
  const handleInputChange = (e: ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    const field = id.replace('form-', '') as keyof FormDataState;
      ...
      <section className="control-section">
        <div className="control-row">
          <div className="control-buttons">
            <button onClick={() => setIsFormOpen(!isFormOpen)}>아기 사자 추가</button>
            <button onClick={onDeleteLast}>마지막 아기 사자 삭제</button>
          </div>
      ...
      <section className={`form-section ${isFormOpen ? '' : 'hidden'}`}>
        <form onSubmit={handleSubmit}>
          <div className="form-group">
            <input type="text" id="form-name" placeholder="이름" value={formData.name} onChange={handleInputChange} />
            <select id="form-part" value={formData.part} onChange={handleInputChange}>
